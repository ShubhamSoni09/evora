...
function authSecret() {
  return process.env.AUTH_SECRET ?? "evora-demo-secret-change-in-production";
}
...
export function verifyPassword(username: string, password: string): boolean {
  ...
  const expected = (process.env[envKey] as string | undefined) ?? DEFAULT_PASSWORDS[user.username];
